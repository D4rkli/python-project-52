...
def test_login_redirects_home(client, django_user_model):
    user = django_user_model.objects.create_user(username="trinity", password="pwd1234567A")
    resp = client.post(reverse("login"), {"username": "trinity", "password": "pwd1234567A"})
    assert resp.status_code == 302
    assert resp.url == "/"

@pytest.mark.django_db
def test_user_can_update_self(client, django_user_model):
    u = django_user_model.objects.create_user(username="morpheus", password="Pwd1234567A")
    client.login(username="morpheus", password="Pwd1234567A")
    resp = client.post(reverse("users_update", args=[u.pk]), {"username": "morpheus2"})
    assert resp.status_code == 302
    u.refresh_from_db()
...
def test_user_cannot_update_others(client, django_user_model):
    u1 = django_user_model.objects.create_user(username="u1", password="Pwd1234567A")
